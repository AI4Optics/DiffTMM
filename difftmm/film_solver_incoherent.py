"""Incoherent / partly-incoherent thin film solver (intensity TMM).

User-facing solver and supporting kernel for stacks containing thick
incoherent layers (e.g., glass substrates beyond the source coherence
length). The intensity-transfer-matrix algorithm wraps the coherent 2x2
solver in :mod:`film_solver_isotropic` for each coherent sub-stack, then
propagates real-valued intensities between incoherent layers.

Public API:
    * :func:`create_intensity_RT_isotropic` — functional incoherent TMM.
    * :class:`IncoherentIsotropicFilmSolver` — stateful solver class.
...
class IncoherentIsotropicFilmSolver(IsotropicFilmSolver):
    ...
    # === Static-method building blocks ============================
    # These methods are pure (no instance state). They're grouped under
    # the class for namespacing and discoverability; ``simulate()`` calls
    # them through the class name. Users may also call them directly,
    # e.g. ``IncoherentIsotropicFilmSolver.group_layers_by_coherence(c_list)``.
    ...
    def _inc_total_RT_for_all_pols(
        ...
        for s_idx in range(num_stacks):
            ...
            Rs_fwd, Rp_fwd, Ts_fwd, Tp_fwd = IncoherentIsotropicFilmSolver.coh_stack_power_RT_isotropic(
            ...
            Rs_bwd, Rp_bwd, Ts_bwd, Tp_bwd = IncoherentIsotropicFilmSolver.coh_stack_power_RT_isotropic(
        ...
        def _interface_RT(inc_i):
            ...
            if nxt_stack is None:
                ...
                Rs_f, Rp_f, Ts_f, Tp_f = IncoherentIsotropicFilmSolver.interface_power_RT(n_a, n_b, cos_a, cos_b)
                Rs_b, Rp_b, Ts_b, Tp_b = IncoherentIsotropicFilmSolver.interface_power_RT(n_b, n_a, cos_b, cos_a)
    ...
    # mat_in, mat_out, mat_ls) is identical. c_list is not persisted,
...
# Preserves ``from difftmm import create_intensity_RT_isotropic``.
# Users may also call IncoherentIsotropicFilmSolver.create_intensity_RT_isotropic
# directly.
create_intensity_RT_isotropic = IncoherentIsotropicFilmSolver.create_intensity_RT_isotropic
